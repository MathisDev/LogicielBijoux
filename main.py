...
class main():
	...
	def __init__(self):
		...
		def showAllClients():
			...
			env = tk.Tk()
			env.title("Clients")
			arbre = ttk.Treeview(env, column=("c1","c2", "c3","c4","c5","c6"), show='headings')
			
			conn = sqlite3.connect('base.db')
			cur = conn.cursor()
			rqt = "SELECT firmeNom, clientNom, clientPrenom, adresseMail, soldeCompte, telephone FROM Clients;"
			cur.execute(rqt)
			rows = cur.fetchall()
			for row in rows:
				print(row) 
		...
		def showAllCommandes():
			...
			env = tk.Tk()
			env.title("Commandes")
			arbre = ttk.Treeview(env, column=("c1", "c2","c3","c4","c5","c6","c7"), show='headings')
			conn = sqlite3.connect('base.db')
			cur = conn.cursor()
			rqt = "SELECT dateDepot, idClient, cp, devis, acompte, resterPaye, description FROM Commandes;"
			cur.execute(rqt)
			rows = cur.fetchall()
			for row in rows:
				print(row) 
		...
		# - Permet de cliquer sur les commandes affichées -
		def get_commandes(event):
			color1 = "#666363"
			fg = 13	
			self.topShowCommande = tk.Toplevel(bg=color1)
			self.topShowCommande.geometry("1500x700")
			tree = event.widget
			citem = tree.focus()
			val = tree.item(citem)
			nom = val["values"][2]
			...
			self.LabelFrames = tk.Label(self.topShowCommande,text=fetch[0],font = fg,bg=color1).place(x=20,y=25)
			self.LabelName = tk.Label(self.topShowCommande,text=fetch[1],bg=color1).place(x=20,y=50)
			self.LabelFirstName = tk.Label(self.topShowCommande,text=fetch[2],bg=color1).place(x=20,y=75)

			self.LabelMail = tk.Label(self.topShowCommande,text=fetch[3],bg=color1).place(x=20,y=110)
			self.LabelNumTel = tk.Label(self.topShowCommande,text=fetch[4],bg=color1).place(x=20,y=135)

			self.LabelCompte = tk.Label(self.topShowCommande,text=fetch[5],bg=color1).place(x=340,y=135)
			self.titreCompte = tk.Label(self.topShowCommande,text='Compte poids : '+fetch[6]+'g',bg=color1).place(x=220,y=135)
		def recuperer_noms():
			conn = sqlite3.connect("base.db")
			cur = conn.cursor()
		...
		tk.Button(window, text='Chercher',highlightbackground = "#666363", command=partial(search,barr_rech)).place(x=900,y=130)
